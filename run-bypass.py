...
def load_token(token_file):
    try:
        ...
        logger.error("Token file not found. Please enter your token:")
        token = input("Enter your token: ").strip()
        if token:
            save_token(token_file, token)
            return token
        else:
            raise SystemExit("Token input was empty, ERROR")
    except Exception as e:
...
if __name__ == '__main__':
    show_warning()
    print("\nAlright, we here! The token is being loaded from 'token.txt'.")
    try:
        asyncio.run(main())
